...
pub async fn analyze_with_gemini(
    ...
    match call_gemini_api(&gemini_api_key, &req.prompt).await {
        ...
        Err(e) => {
            ...
            Ok(
                HttpResponse::InternalServerError().json(GeminiAnalysisResponse {
                    success: false,
                    analysis: None,
...
async fn call_gemini_api(
    ...
    println!(
        "Gemini API text extracted successfully - Length: {} chars",
        text.len()
    );

    // Extract token usage information
    let token_usage = response_json.get("usageMetadata").map(|usage| {
...
pub async fn test_gemini_api(data: web::Data<std::sync::Arc<ApiState>>) -> Result<HttpResponse> {
    let (api_key_present, gemini_api_key) = {
        let config_guard = data.config.lock().unwrap();
        let api_key_present = !config_guard.gemini_api_key.is_empty()
            && config_guard.gemini_api_key != "dummy_key"
    ...
    .await
    {
        ...
        Err(e) => Ok(HttpResponse::Ok().json(GeminiTestResponse {
            success: false,
